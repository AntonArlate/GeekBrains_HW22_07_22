...
while (begin)
{
    ...
    Console.WriteLine("   55.\t Заполнит спирально массив 4 на 4. (не верная версия)");
...
void CaseProgramm55()
{
    ...
    FillArray(array, row, colum, count);

    void FillArray(int[,] array, int row, int colum, int count)
    {
        array[row, colum] = count;
        count++;
        if (colum + 1 < array.GetLength(1) && array[row, colum + 1] == 0)
        {
            FillArray(array, row, colum + 1, count);
        }
        if (row + 1 < array.GetLength(0) && array[row + 1, colum] == 0)
        {
            FillArray(array, row + 1, colum, count);
        }
        if (colum - 1 >= 0 && array[row, colum - 1] == 0)
        {
            FillArray(array, row, colum - 1, count);
        }
        if (row - 1 >= 0 && array[row - 1, colum] == 0)
        {
            FillArray(array, row - 1, colum, count);
        }
    }
